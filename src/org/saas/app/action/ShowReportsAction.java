...
public class ShowReportsAction extends ActionSupport{
	...
	//执行对应的SQL语句，取得相应的结果
	public String doQuery(String sql) throws SQLException{
		//新建连接
		try{
			new SQLConn();
		}
	...
	public String execute() throws Exception{
		...
		if(checkTypes.equals("流水<>实收+优惠")){
			sql = "select saasOrderKey as [订单号],"
			+ "foodAmount as [流水],"
			+ "promotionAmount as [优惠金额],"
			+ "paidAmount as [实收金额],"
			+ "round((foodAmount - promotionAmount - paidAmount),2) as [差值] from(select saasOrderKey,foodAmount,promotionAmount,paidAmount,round((foodAmount - promotionAmount - paidAmount),2) from tbl_saas_order_master where reportDate between "
			+ getStartDate() + " and "
			+ getEndDate() + " and round(foodAmount,4) <> round((promotionAmount + paidAmount),4) and orderStatus = 40 union all select saasOrderKey,foodAmount,promotionAmount,paidAmount,round((foodAmount - promotionAmount - paidAmount),2) from tbl_saas_order_master_his where reportDate between "
			+ getStartDate() + " and "
			+ getEndDate() + " and round(foodAmount,4) <> round((promotionAmount + paidAmount),4) and orderStatus = 40)";
